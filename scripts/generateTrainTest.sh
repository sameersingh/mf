...
awk -v subset="$subsetSize" 'subset<0||FNR<subset' $train_files > "${basePath}data/train_matrix.subset"
awk -v subset="$subsetSize" 'subset<0||FNR<subset' $test_files > "${basePath}data/test_matrix.subset"